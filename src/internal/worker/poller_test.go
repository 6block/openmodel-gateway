...
// B1: /ready may carry seconds_until_change BEFORE gpu_state (order matters for
// backward compat — old parsers take everything after "gpu_state=").
func TestFetchGPUStateParsesUntilChange(t *testing.T) {
	newFmt := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	oldFmt := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
