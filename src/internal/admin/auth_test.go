...
func TestAuthMiddleware_PlainTokenRejected(t *testing.T) {
	// G2: a raw token WITHOUT the "Bearer " scheme must now be rejected.
	handler := authMiddleware("secret-token", testHandler())
	...
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusUnauthorized {
		t.Errorf("expected 401 (plain token rejected), got %d", rec.Code)
	}
}

func TestAuthMiddleware_LongWrongTokenRejected(t *testing.T) {
	// G2: differing-length tokens must still be rejected (and compared in
	// constant time via hashing — no length short-circuit).
	handler := authMiddleware("secret-token", testHandler())

	req := httptest.NewRequest(http.MethodGet, "/api/v1/workers", nil)
	req.Header.Set("Authorization", "Bearer "+stringRepeat("x", 4096))
	rec := httptest.NewRecorder()
	handler.ServeHTTP(rec, req)
	if rec.Code != http.StatusUnauthorized {
		t.Errorf("expected 401, got %d", rec.Code)
